...
-- 4. Obtener el valor del atributo netapa de aquellas etapas con salida y llegada en la misma ciudad.
	select netapa from etapa where etapa.salida=etapa.llegada;
...
-- 16. Nombre de los ciclistas que han llevado el maillot General.
	select distinct llevar.codigomail, ciclista.nombre from ciclista inner join llevar on ciclista.dorsal=llevar.dorsal and llevar.codigomail='MGE';
...
-- 35. Obtener el valor del atributo netapa de las etapas que no comienzan en la misma ciudad en que acabó la anterior etapa.
	select etapa.netapa from etapa inner join etapa on netapa=netapa where exists (select ) 
